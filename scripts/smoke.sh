...
node -e "const fs=require('node:fs'); const m=JSON.parse(fs.readFileSync(process.argv[1],'utf8')); if (m.queued.length !== 2 || m.written.length !== 2) process.exit(1)" "$OUT/manifest.json"
